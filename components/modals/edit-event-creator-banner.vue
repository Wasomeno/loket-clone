...
async function updateLogo() {
  ...
  await axios.patch(`${EVENT_API_MAIN}/creators/${props.creatorId}/banner`, {
...
<template>
  <Modal
    ...
    class="flex flex-1 flex-col items-center bg-white lg:h-4/6 lg:w-4/6"
  >
    <div class="flex w-full items-center justify-between border-b px-6 py-4">
      <h5 class="text-sm font-medium lg:text-base">Edit Banner</h5>
      <button @click="$emit('close-modal')" class="h-4 w-4 font-medium">
        <CIcon :icon="cilX" />
      </button>
    </div>
    <div class="flex flex-1 flex-col items-center justify-center">
      <div class="h-5/6 w-full overflow-hidden rounded-lg border-2 shadow-sm">
